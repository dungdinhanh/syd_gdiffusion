...
from guided_diffusion_hfai.train_util import parse_resume_step_from_filename, log_loss_dict, model_entropy, NormalNLLLoss
...
def main(local_rank):
    ...
    def forward_backward_log(data_loader, prefix="train"):
        ...
        for i, (sub_batch, sub_batch_noises, sub_gt_labels, sub_t) in enumerate(
            ...
            # continuous loss
            mu_clean = clean_logits[:, (cat_dim * num_cat): ((cat_dim * num_cat) + num_con)].detach()
            var_clean = torch.exp(clean_logits[:, ((cat_dim * num_cat) + num_con):].detach())
            ...
            loss_nll = nll_loss(x_clean, mu_noise, var_noise)
            ...
            loss = loss_ce + alpha_e * loss_entropy + loss_nll
            ...
            losses[f"{prefix}_loss_mse"] = loss_nll.detach()
