...
def main():
    ...
    def forward_backward_log(data_loader, prefix="train"):
        ...
        for i, (sub_batch, sub_batch_noises, sub_gt_labels, sub_t) in enumerate(
            ...
            loss_kd_transfer = kdloss(clean_logits[:, :(cat_dim * num_cat)], clean_disc.detach())
            # mse loss
            ...
            loss_mse_transfer = torch.mean(F.mse_loss(logits_mu_var,
                                             q_mu_var,
                                             reduction='none'), dim=1)
            # guidance loss
            noise_logits = model(sub_batch_noises, timesteps=sub_t)

            loss_ce_guidance = kdloss(noise_logits[:, :(cat_dim * num_cat)],
            ...
            # x_clean = mu_clean + th.rand_like(var_clean).to(dist_util.dev()) * var_clean
            loss_con_guidance = torch.mean(F.mse_loss(logits_mu_var_noise, logits_mu_var.detach(), reduction='none'), dim=1)
