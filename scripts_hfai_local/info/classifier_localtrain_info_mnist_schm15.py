...
def main():
    ...
    output_channels = num_con + cat_dim * num_cat
    ...
    def forward_backward_log(data_loader, prefix="train"):
        ...
        for i, (sub_batch, sub_batch_noises, sub_gt_labels, sub_t) in enumerate(
            ...
            loss_nll = nll(noise_logits[:, (cat_dim * num_cat):], q_mu, q_var)
            ...
            loss_con_guidance = torch.mean(F.mse_loss(noise_logits[:, (cat_dim * num_cat):],
                                                      clean_logits[:, (cat_dim * num_cat):].detach(),
                                                      reduction='none'),
                                           dim=1)

            loss = loss_kd + loss_nll + loss_ce_guidance + loss_con_guidance

            losses = dict()

            losses[f"{prefix}_loss"] = loss.detach()
            losses[f"{prefix}_loss_kd"] = loss_kd.detach()
            losses[f"{prefix}_loss_nll"] = loss_nll.detach()
            losses[f"{prefix}_loss_ce_guide"] = loss_ce_guidance.detach()
            losses[f"{prefix}_loss_con_guide"] = loss_con_guidance.detach()
