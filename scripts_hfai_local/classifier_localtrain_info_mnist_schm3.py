...
def main():
    ...
    def forward_backward_log(data_loader, prefix="train"):
        ...
        for i, (sub_batch, sub_batch_noises, sub_gt_labels, sub_t) in enumerate(
            ...
            loss_ce = F.cross_entropy(noise_logits, torch.argmax(clean_logits.detach(), 1).to(dist_util.dev()), reduction="none")

            loss_entropy = model_entropy(F.softmax(clean_logits, dim=1))
            # loss = kdloss_gb(noise_logits, clean_logits.detach())

            # loss = kdloss(clean_logits, noise_logits.detach())
            loss = loss_ce + alpha_e * loss_entropy
