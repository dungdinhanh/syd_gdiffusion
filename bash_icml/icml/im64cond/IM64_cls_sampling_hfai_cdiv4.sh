...
scales=( "001" "0001" "00001" )

for scale in "${scales[@]}"
do
cmd="python scripts_hfai_gdiff/multitask/classifier_sample_mlt_cdiv2.py $MODEL_FLAGS --classifier_scale 0.${scale}  \
--classifier_path models/64x64_classifier.pt --model_path models/64x64_diffusion.pt $SAMPLE_FLAGS \
 --logdir runs/sampling_icml/IMN64/conditional/scale0p${scale}/ --classifier_depth 4"
echo ${cmd}
eval ${cmd}
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet64_labeled.npz \
 runs/sampling_icml/IMN64/conditional/scale0p${scale}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
