...
for scale in "${scales[@]}"
do
cmd="python scripts_hfai_gdiff/multitask/classifier_sample_mlt_cdiv2.py $MODEL_FLAGS --classifier_scale ${scale}.0 --classifier_path models/128x128_classifier.pt \
--model_path models/128x128_diffusion.pt $SAMPLE_FLAGS  --logdir runs/sampling_icml/IMN128/conditional/scale${scale}p0/"
echo ${cmd}
eval ${cmd}
done
