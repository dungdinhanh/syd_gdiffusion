...
for scale in "${scales[@]}"
do
cmd="python scripts_hfai_gdiff/multitask/classifier_sample_mlt2.py $MODEL_FLAGS --classifier_scale 0.${scale} --classifier_path models/128x128_classifier.pt \
--model_path models/128x128_diffusion.pt $SAMPLE_FLAGS  --logdir runs/sampling_icml/IMN128/conditional_cdiff/scale0p${scale}/"
#echo ${cmd}
#eval ${cmd}
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet128_labeled.npz \
 runs/sampling_icml/IMN128/conditional_cdiff/scale0p${scale}/reference/samples_50000x128x128x3.npz"
echo ${cmd}
eval ${cmd}
done
