...
for scale in "${scales[@]}"
do
cmd="python scripts_hfai_gdiff/classifier_sample.py $MODEL_FLAGS --classifier_scale 0.${scale} --classifier_path models/128x128_classifier.pt \
--model_path models/128x128_diffusion.pt $SAMPLE_FLAGS  --logdir runs/sampling_icml/IMN128/conditional_nomlt/scale0p${scale}/"
echo ${cmd}
eval ${cmd}
done
