...
for scale in "${scales[@]}"
do
cmd="python scripts_hfai_gdiff/classifier_free/classifier_free_sample_mlt3.py $MODEL_FLAGS --cond_model_scale ${scale}  \
...
--model_path runs/IM64/IM64_diffusion_training_unconditional/models/ema_0.9999_540000_final.pt  $SAMPLE_FLAGS \
 --logdir runs/sampling_icml_classifier_free/IMN64/mlt_delta1wdelta/scale${scale}/ "
echo ${cmd}
eval ${cmd}
done
...
cmd="python evaluations/evaluator_tolog.py reference/VIRTUAL_imagenet64_labeled.npz \
 runs/sampling_icml_classifier_free/IMN64/mlt_delta1wdelta/scale${scale}/reference/samples_50000x64x64x3.npz"
echo ${cmd}
eval ${cmd}
done
