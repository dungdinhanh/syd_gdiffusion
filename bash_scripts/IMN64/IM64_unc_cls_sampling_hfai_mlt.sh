...
for scale in "${scales[@]}"
do
cmd="python scripts_hfai_gdiff/classifier_sample_mlt.py $MODEL_FLAGS --classifier_scale ${scale}.0  \
--classifier_path models/64x64_classifier.pt \
 ...
 $SAMPLE_FLAGS --logdir runs/sampling/IMN64/unconditional_mlt/scale${scale}p0/ --classifier_depth 4"
echo ${cmd}
eval ${cmd}
done

cmd="python scripts_hfai_gdiff/classifier_sample_mlt.py $MODEL_FLAGS --classifier_scale 0.5  \
--classifier_path models/64x64_classifier.pt \
--model_path runs/IM64/IM64_diffusion_training_unconditional/models/ema_0.9999_540000_final.pt \
 $SAMPLE_FLAGS --logdir runs/sampling/IMN64/unconditional_mlt/scale0p5/ --classifier_depth 4"
echo ${cmd}
eval ${cmd}
