...
for scale in "${scales[@]}"
do
cmd="python scripts_hfai_gdiff/multitask/classifier_sample_mlt_channelconflict.py $MODEL_FLAGS --classifier_scale ${scale}.0  \
--classifier_path models/64x64_classifier.pt \
 ...
 $SAMPLE_FLAGS --logdir runs/sampling2/IMN64/unconditional_mlt2_chconf/scale${scale}p0/ --classifier_depth 4"
echo ${cmd}
eval ${cmd}
done

cmd="python scripts_hfai_gdiff/multitask/classifier_sample_mlt_channelconflict.py $MODEL_FLAGS --classifier_scale 0.5  \
--classifier_path models/64x64_classifier.pt \
--model_path runs/IM64/IM64_diffusion_training_unconditional/models/ema_0.9999_540000_final.pt \
 $SAMPLE_FLAGS --logdir runs/sampling2/IMN64/unconditional_mlt2_chconf/scale0p5/ --classifier_depth 4"
echo ${cmd}
eval ${cmd}
