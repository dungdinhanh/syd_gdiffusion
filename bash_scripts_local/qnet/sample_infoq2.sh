...
name="sampling_dbglarge_s"

MODEL_FLAGS="--image_size 28 --num_channels 64 --num_res_blocks 1 --attention_resolutions 7,4 --diffusion_steps 1000 --dropout 0.1 --noise_schedule cosine\
 --num_head_channels 64 --resblock_updown True --use_new_attention_order True --use_fp16 True --use_scale_shift_norm True --learn_sigma True"
...
--model_path ${mpath} --logdir ${opath}/${name}_ddpm_ema4/ --classifier_scale 4.0 \
...
cmd="${cd_cmd} python scripts_hfai_local/classifier_sample_infoq_local.py --classifier_path ${cpath} \
--model_path ${mpath} --logdir ${opath}/${name}_ddpm_ema5/ --classifier_scale 5.0 \
--classifier_attention_resolutions 14,7 --classifier_depth 1 \
--classifier_width 32 --classifier_pool attention --classifier_resblock_updown True --classifier_use_scale_shift_norm True \
 $MODEL_FLAGS ${SAMPLE_FLAGS1}"
echo ${cmd}
...
cmd="${cd_cmd} python scripts_hfai_local/classifier_sample_infoq_local.py --classifier_path ${cpath} \
--model_path ${mpath} --logdir ${opath}/${name}_ddpm_ema7/ --classifier_scale 7.0 \
--classifier_attention_resolutions 14,7 --classifier_depth 1 \
--classifier_width 32 --classifier_pool attention --classifier_resblock_updown True --classifier_use_scale_shift_norm True \
 $MODEL_FLAGS ${SAMPLE_FLAGS1}"
echo ${cmd}
...
cmd="${cd_cmd} python scripts_hfai_local/classifier_sample_infoq_local.py --classifier_path ${cpath} \
--model_path ${mpath} --logdir ${opath}/${name}_ddpm_ema10/ --classifier_scale 10.0 \
--classifier_attention_resolutions 14,7 --classifier_depth 1 \
--classifier_width 32 --classifier_pool attention --classifier_resblock_updown True --classifier_use_scale_shift_norm True \
 $MODEL_FLAGS ${SAMPLE_FLAGS1}"
echo ${cmd}
